...
7. Add or update unit tests in `src/tests/unit/`.
8. Add integration tests when CLI, file I/O, or metadata execution behavior changes.
9. Run targeted validation: `python -m pytest src/tests/unit/<path>` and `python -m flake8 src/syngen/<path>`.
10. Invoke `syngen-code-reviewer` and `syngen-test-engineer` for non-trivial changes.
11. Invoke `syngen-security-reviewer` when encryption, credentials, or data flow changes.
12. Provide final handoff: changed files, tests run, results, skipped checks with reason, residual risks.
