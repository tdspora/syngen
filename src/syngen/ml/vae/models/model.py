...
class CVAE:
    ...
    def __init__(
        self, dataset, batch_size, latent_dim, intermediate_dim, latent_components
    ):
        self.dataset = dataset
        self.intermediate_dim = intermediate_dim
        self.batch_size = batch_size
        self.latent_dim = latent_dim
        self.latent_components = min(
            latent_components, len(self.dataset.order_of_columns)
        )
        self.model = None
    ...
    def fit_sampler(self, data: pd.DataFrame):
        ...
        self.latent_model = BayesianGaussianMixture(
            n_components=self.latent_components,
            n_init=10,
        )
    ...
    def __check_pk_numeric_convertability(self, column, key_type):
        if key_type is str and column not in (
            self.dataset.long_text_columns | self.dataset.uuid_columns
        ):
    ...
    def __make_pk_uq_unique(self, pk_uq_keys_mapping, empty_columns):
        for key_name, config in pk_uq_keys_mapping.items():
            ...
            for column in key_columns:
                ...
                if key_type is float or self.__check_pk_numeric_convertability(
                    column, key_type
                ):
                    mapped_keys = (
