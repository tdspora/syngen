...
class BaseTest(ABC):
    ...
    def update_progress_bar(delta, message):
        ...
        progress_bar.set_progress(
            progress=progress_bar.progress + delta,
            delta=None,
            message=message
        )
...
class AccuracyTest(BaseTest):
    ...
    def _fetch_metrics(self, **kwargs):
        ...
        if self.plot_exists:
            ...
            self.update_progress_bar(delta, "The univariate distributions have been generated")

            self.update_progress_bar(0, "Generation of the bivariate distributions...")
            bi_images = self.bivariate.calculate_all(
                kwargs["cont_columns"], kwargs["categ_columns"], kwargs["date_columns"]
            )
            self.update_progress_bar(delta, "The bivariate distributions have been generated")
