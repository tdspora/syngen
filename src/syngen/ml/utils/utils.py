...
import importlib
import re
from typing import List, Dict, Optional, Union, Set, Tuple, Literal, Callable
...
def get_function_from_namespace(function_name: str) -> Callable:
    """
    The helper function to get a function object from the caller's namespace.
    ...
    raise NameError(
        ...
        f"Please import it first or provide the full path 'module.path:function_name'."
    )


def get_function_from_string(function_path: str) -> Callable[[str], pd.DataFrame]:
    """
    Import a function from a string path in the format 'module.path:function_name' or
    just get the object of the function from the function path

    Parameters
    ----------
    function_path : str
        The path to the function in format 'module.path:function_name'
        or just the name of the function
        Example: 'my_module.loaders:custom_csv_loader' or 'custom_csv_loader'
    ...
    AttributeError
        If the function isn't found in the module
    TypeError
        If the imported object is not callable
    """
    info_message = "the value of the parameter '--loader' is provided"
    module_path = str()
    function_name = str()
    ...
    try:
        if ":" in function_path:
            ...
            logger.debug(
                f"As {info_message} in the format 'module.path:function_name', "
            ...
            func = get_function_from_namespace(function_name)

        if not callable(func):
            raise TypeError(
                f"{info_message.capitalize()}, but it seems "
                f"that the object '{function_name}' from module '{module_path}' is not callable."
            )

        logger.info(f"The custom loader function '{function_name}' is successfully provided.")

        return func

    except ModuleNotFoundError as e:
        raise ModuleNotFoundError(
            f"{info_message.capitalize()} in the format 'module.path:function_name', "
            f"but it's failed to import module '{module_path}': {str(e)}."
        )
    except AttributeError as e:
        raise AttributeError(
            f"{info_message.capitalize()} in the format 'module.path:function_name', "
            f"but the function '{function_name}' isn't found "
            f"in the module '{module_path}': {str(e)}."
        )


def get_loader_function(loader_path: Optional[str]) -> Optional[Callable[[str], pd.DataFrame]]:
    """
    Get a loader function from a string path, or return None if not provided

    Parameters
    ----------
    loader_path : Optional[str]
        Path to the function in format 'module.path:function_name'
    ...
    return get_function_from_string(loader_path)
