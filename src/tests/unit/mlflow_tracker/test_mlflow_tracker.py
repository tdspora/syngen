...
def test_log_artifact_with_active_mlflow(mlflow_tracker, rp_logger):
    ...
    with patch(
        ...
        mock_log_artifact.assert_called_once_with(
            "path/to/local/storage", "path/to/artifacts"
        )
    rp_logger.info(SUCCESSFUL_MESSAGE)
...
def test_create_experiment_with_active_status(mlflow_tracker, rp_logger):
    ...
    with patch(
        ...
        mock_create_experiment.assert_called_once_with(
            test_experiment_name, artifact_location
        )
    rp_logger.info(SUCCESSFUL_MESSAGE)
...
@patch(
    "syngen.ml.mlflow_tracker.mlflow_tracker.mlflow.search_experiments", return_value=[]
)
def test_set_new_experiment_without_env_var_and_with_active_mlflow(
    mock_search_experiment,
    ...
    rp_logger.info(
        ...
        "for the new experiment without setting up 'MLFLOW_EXPERIMENT_NAME' "
    ...
    with caplog.at_level(level="WARNING"):
        ...
        assert (
            "A new experiment with the name based on 'table_name' or 'metadata_path' value - "
            "'test_experiment_third' will be created"
        ) in caplog.text
        mock_create_experiment.assert_called_once_with(
...
def test_set_new_experiment_with_env_var_and_with_active_mlflow(
    ...
    rp_logger.info(
        "Test the method 'set_experiment' of the class 'MlflowTracker' "
        "for the new experiment with setting up 'MLFLOW_EXPERIMENT_NAME' "
        "and with the active mlflow"
    )
    ...
    with caplog.at_level(level="INFO"):
        ...
        assert (
            "A new experiment with the name - 'test_experiment_sixth' will be created"
            in caplog.text
        )
        mock_create_experiment.assert_called_once_with(
            test_experiment_name, "/mlflow_tracker"
...
def test_create_mlflow_tracker_for_single_table(
    ...
    MlflowTrackerFactory.create_tracker(table_name="table_new", metadata_path=None)
...
def test_create_mlflow_tracker_for_metadata_path(
    ...
    MlflowTrackerFactory.create_tracker(
        table_name=None, metadata_path="path/to/metadata.yaml"
    )
...
def test_create_mlflow_tracker_with_inactive_server(
    ...
    monkeypatch.setenv("MLFLOW_TRACKING_URI", "http://mock_server:5000")
    MlflowTracker._instance = None
    MlflowTrackerFactory.create_tracker(table_name="test_table", metadata_path=None)
...
def test_get_mlflow_exp_name_if_env_var_not_present(monkeypatch, rp_logger):
    ...
    assert MlflowTrackerFactory.get_mlflow_exp_name("test_table", None) == "test-table"
    rp_logger.info(SUCCESSFUL_MESSAGE)
